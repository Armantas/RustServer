...
function PLUGIN:cmdCheck( netuser, cmd, args )
	...
	rust.Notice( netuser, targetuser.displayName .. " is being checked" )
	self.Data[userID] = {}
...
function PLUGIN:CheckFallSpeed(netuser,userID,current)
	...
	if(delta < 30 and delta > 0.001) then 
		...
		if(self.Config.SendDetectionsToAdmins) then
			self:SendToAdmins( netuser.displayName .. " is falling only by " .. delta .. " meters" )
		end
	elseif(coords.y > 10004) then
		self.Data[userID].detected = self.Data[userID].detected + 10
		if(self.Config.SendDetectionsToAdmins) then
			self:SendToAdmins( netuser.displayName .. " is going up in the sky while getting checked! Nice Flyhack!" )
		end
	end
	...
	if(self.Data[userID].detected >= self.Config.NumberOfDetectionsBeforeBan) then
		if(self.Config.AllowAutoBan) then
			...
			if(self.Config.BroadcastBan) then
				rust.BroadcastChat( self.Config.chatName, "[color #FF4444]" .. netuser.displayName .. " has been banned for FlyHack")
			end
			return
		end
	end 
	if(current > self.Config.TimerToCheckForFallDamage) then
		rust.SendChatToUser(netuser, self.Config.chatName, "You have been cleared of flyhack, sorry for this rude entry!!")
		rust.Notice(netuser, "You have been cleared of flyhack, sorry for this rude entry!!")
		...
		timer.NextFrame( function()
			timer.Once( 1, function()
				if(netuser and netuser.playerClient) then
					...
					rust.SendChatToUser(netuser, self.Config.chatName, "All your stuff was given back to you!!")
				end
			end )
		end)
	end
...
function PLUGIN:LoadDefaultConfig()
	self.Config.chatName = "Anti-Flyhack"
