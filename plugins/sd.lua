...
function PLUGIN:Init()
	...
	self:AddChatCommand("sdh", self.ShowDamageHelpCmd)
	self:AddChatCommand("showdamagehelp", self.ShowDamageHelpCmd)
...
function PLUGIN:ShowDamageHelpCmd(netuser,args)
...
function PLUGIN:OnHurt(takedamage, damage)
	...
	if (takedamage:GetComponent("HumanController")) then
		if(damage.victim.client) then
			...
			local typeofdamage = tostring(damage.damageTypes)
			if (typeofdamage == tostring(DamageType.damage_bullet) or typeofdamage == tostring(DamageType.damage_explosion) or typeofdamage == tostring(DamageType.damage_melee)) then -- thanks to Rick(aka mrshl9pa)
...
function PLUGIN:OnKilled( takedamage, damage )
	if (takedamage:GetComponent("HumanController")) then
		...
		if (damage.victim.client) then
			if (damage.attacker.client.netUser and damage.victim.client.netUser) then
				local typeofdamage = tostring(damage.damageTypes)
				if (typeofdamage == tostring(DamageType.damage_bullet) or typeofdamage == tostring(DamageType.damage_explosion) or typeofdamage == tostring(DamageType.damage_melee)) then
					steamidDmg = rust.CommunityIDToSteamID( tonumber(rust.GetUserID(damage.attacker.client.netUser )))
...
function PLUGIN:SendHelpText( netuser )
	rust.SendChatToUser( netuser, "Use /showdamagehelp or /sdh to show showdamage help menu." )
end